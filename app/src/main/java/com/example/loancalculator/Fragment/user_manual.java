...
public class user_manual extends Fragment {
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    // 🔹 Purity Info Dialog
    textViewPurityInfo.setOnClickListener(
        v -> {
          new AlertDialog.Builder(requireContext())
              .setTitle("How Gold Purity Works")
              .setMessage(
                  "Different lenders deduct impurities based on their own policies. This app lets"
                      + " you customize purity deductions to match your jeweller or bank’s"
                      + " calculation.\n\n"
  ...
  private void checkForUpdate() {
    versionRef.addListenerForSingleValueEvent(
        new ValueEventListener() {
          ...
          public void onDataChange(@NonNull DataSnapshot snapshot) {
            if (snapshot.exists()) {
              ...
              Toast.makeText(
                      getContext(),
                      "Version info initialized: " + CURRENT_VERSION,
                      Toast.LENGTH_SHORT)
                  .show();
            }
          }
  ...
  private void fetchDownloadLinkAndPrompt(String latestVersion) {
    ...
    ref.get()
        ...
        .addOnFailureListener(
            e ->
                Toast.makeText(
                        getContext(), "Failed to fetch link: " + e.getMessage(), Toast.LENGTH_SHORT)
                    .show());
  }
}
