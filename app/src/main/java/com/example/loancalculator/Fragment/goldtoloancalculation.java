...
public class goldtoloancalculation extends Fragment {
    ...
    new Thread(
            () -> {
              ...
              if (settings != null) {
                ...
                requireActivity()
                    .runOnUiThread(() -> btnBasePrice.setText("Base LTV : ₹" + (int) baseLtvPrice));
              }
            })
        .start();
  }
  ...
  private void calculatePureGold(String purityLabel) {
    ...
    try {
      ...
      if (netWeight < 0) {
        Toast.makeText(getContext(), "Stone weight cannot exceed gold weight", Toast.LENGTH_SHORT)
            .show();
        return;
      }
    ...
    new Thread(
            () -> {
              ...
              requireActivity()
                  .runOnUiThread(
                      () -> {
                        LinearLayout schemeContainer =
                            requireView().findViewById(R.id.schemeContainer);
                        ...
                        for (Scheme scheme : schemes) {
                          ...
                          double cappedLoan =
                              Math.min(
                                  Math.max(baseLoan, scheme.getMinLimit()), scheme.getMaxLimit());

                          // Skip if below min
                          if (baseLoan < scheme.getMinLimit()) continue;

                          // ✅ Skip if existing loan is higher than scheme's max limit
                          if (finalExistingLoan > 0 && finalExistingLoan > scheme.getMaxLimit())
                            continue;
                          ...
                          double totalLoan =
                              finalExistingLoan > 0
                                  ? finalExistingLoan + additionalLoan
                                  : cappedLoan;
                          ...
                          View cardView =
                              LayoutInflater.from(getContext())
                                  .inflate(R.layout.scheme_card, schemeContainer, false);
                          ...
                          TextView tvMonthlyInterest =
                              cardView.findViewById(R.id.tvMonthlyInterest);
                          ...
                          animeffect.animateLoanAmount(
                              tvLoanAmount, (int) (totalLoan / 2), (int) totalLoan);
                          ...
                          // Animation
                          Animation anim =
                              AnimationUtils.loadAnimation(getContext(), R.anim.slide_up_fade_in);
