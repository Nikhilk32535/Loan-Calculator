...
public class addscheme extends Fragment {
    ...
    try {
      ...
      new Thread(
              () -> {
                ...
                FirebaseDatabase.getInstance().getReference("BaseLTV").setValue(current75LtvPrice);
                          ...
                          requireActivity()
                              .runOnUiThread(
                                  () -> {
                                    ...
                                    showToast(
                                        "✅ Base LTV updated. Schemes refreshed online & offline.");
                                  });
                        });
              })
          .start();
  ...
  private void saveScheme() {
    ...
    if (TextUtils.isEmpty(name)
        ...
        || TextUtils.isEmpty(minLimitStr)
        || TextUtils.isEmpty(maxLimitStr)) {
  ...
  private void uploadSchemeToFirebase(Scheme scheme) {
    FirebaseDatabase.getInstance()
        ...
        .addOnSuccessListener(
            aVoid ->
                requireActivity()
                    .runOnUiThread(() -> showToast("\u2705 Scheme also uploaded online")))
  ...
  private int calculatePriceByLtvType(String ltvType, float base75) {
    switch (ltvType) {
      case "50LTV":
        return (int) ((base75 / 75f) * 50f);
      case "60LTV":
        return (int) ((base75 / 75f) * 60f);
      case "65LTV":
        return (int) ((base75 / 75f) * 65f);
      case "75LTV":
      default:
        return (int) base75;
    }
  ...
  private void setupRecyclerView() {
    ...
    schemeAdapter.setOnDeleteClickListener(
        scheme ->
            new Thread(
                    () -> {
  ...
  private void uploadAllLocalSchemesToFirebase() {
    new Thread(
            () -> {
              ...
              requireActivity()
                  .runOnUiThread(() -> showToast("All local schemes uploaded to Firebase"));
            })
        .start();
  }
                                  ...
                                  .runOnUiThread(
                                      () -> {
                                        if (et75LtvPrice != null) {
                                          et75LtvPrice.setHint(
                                              String.valueOf((int) current75LtvPrice));
                                        }
                                      });
                            }
                          })
